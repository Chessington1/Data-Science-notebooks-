...
class FieldDataProcessor:
    ...
    def ingest_sql_data(self):
        ...
        try:
            ...
            logger.info(
                f"Successfully loaded data. Shape: {self.df.shape}"
            )
    ...
    def rename_columns(self):
        ...
        if 'Annual_yield' in self.df.columns and 'Crop_type' in self.df.columns:
            self.df.rename(
                columns={'Annual_yield': 'Crop_type_Temp',
                         'Crop_type': 'Annual_yield'},
                inplace=True
            )
            self.df.rename(
                columns={'Crop_type_Temp': 'Crop_type'},
                inplace=True
            )
    ...
    def merge_with_weather_mapping(self, weather_mapping_df: pd.DataFrame):
        """
        Merge field dataframe with weather mapping.
        """
        ...
        logger.info(
            f"Merged field data with weather mapping. "
            f"New shape: {self.df.shape}"
        )
    ...
    def process(self, weather_df=None):
        """
        Run all cleaning steps in a consistent order.
        """
        self.rename_columns()
